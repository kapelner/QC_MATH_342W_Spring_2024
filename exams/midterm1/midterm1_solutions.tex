...
\inred{
\beqn
&& s^2_y = \oneover{n-1} \sum_{i=1}^n (y_i - \ybar)^2 = \oneover{n-1} SST \\
&& \Rightarrow~ SST = (n-1) s^2_y = (506-1) \times 84.58672 = 42716.29
\eeqn
The numbers 506 and 84.58672 are found in the code above.
}
\subquestionwithpoints{3} Compute SSE to the nearest two decimals.

\inred{
\beqn
&& s^2_e = \oneover{n-1} \sum_{i=1}^n (e_i - \bar{e})^2 = \oneover{n-1} \sum_{i=1}^n e_i^2 = \oneover{n-1} SSE\\
&& ~\Rightarrow~ SSE = (n-1) s^2_e = (506-1) \times 21.93819 = 11078.79
\eeqn
The numbers 506 and 21.93819 are found in the code above.
}
\pagebreak
...
\inred{\footnotesize
From $j=1$ to 14 features, this plot will be nearly identical to the in-sample plot on the previous page as there is not too much estimation error in $n_{\text{train}} \approx 450$ with $p+1 = 14$ slope coefficients to estimate. If you want to make the $R^2$'s slightly lower here it would not be wrong. The stark difference is from $j=15$ to 506 features. Here we are overfitting the noise and the noise during training will be different than noise during test and hence performance will be degraded. Here, the $R^2$ will be monotonically decreasing and could very well be negative. As long as this piece was shown decreasing, full credit.
}
\normalsize

\begin{figure}[htp]
\centering
\includegraphics[width=5.5in]{out_of_sample}
\end{figure}
\eenum
...
\inred{
Zero (for any reasonably defined error metric)
}
...
\inred{
We know that $\H$, the orthogonal projection matrix, can be equivalently computed as $\Q\Q^\top$. Thus $\Q\Q^\top\y = \yhat$. As this is the orthogonal projection, there is the remainder $\e$ so that $\yhat + \e = \y$ and $\yhat^\top \e = 0$. Thus, $\y, \yhat, \e$ form a right triangle and by Pythagorean Theorem, $\normsq{\y} = \normsq{\yhat} + \normsq{\e}$. Putting these facts together we have:
...
\inred{
Ignorance. The $\x_{\cdot j}$'s are all random noise and thus they absolutely will not be good proxies for the true proximal causal drivers.
}
