...
\subquestionwithpoints{2} Given the join in the previous question, what would be the number of rows in the final joined table after listwise deletion of missingness? \inred{5}
...
\item[\inred{\fbox{e}}] In boosting at iteration $t$, $\mathbb{D}_t$ is updated with the results of $g_1, g_2, \ldots, g_{t-1}$ 
...
\item[\inred{\fbox{i}}] In boosting, the $M$ base learners are fit on the the same input matrices i.e. \\ $\X_1 = \X_2 = \ldots = \X_M$ 
...
\inred{We begin with the intercept (so that doesn't count as an \qu{iteration}. Then, there's $p$ linear terms, $p$ squared terms and $\binom{p}{2}$ interactions of two different variables, i.e., $ p + p + \binom{p}{2} = 26 + \binom{13}{2} = 104$.}
