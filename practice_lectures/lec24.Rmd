...
After downloading the images, for each image we need to (a) transform its representation to a tensor (b) resize each image to be standard 224x224 and (c) normalize the pixel values.
...
full_data_set = dogs_vs_cats_dataset(
  ...
  transform = function(img) {
    img %>%
      ...
      torchvision::transform_normalize(mean = rep(0.5, 3), std = rep(0.5, 3))
  },
  target_transform = function(x){
    as.double(x) - 1 #this converts y to zero/one
  }
)
```
...
cnn_mod = net %>%
  ...
  luz::fit(D_train, epochs = num_iter, valid_data = D_select)
```
...
y_hat_bin = as.numeric(phat) > 0.5
oos_conf_tab = table(y_test, y_hat_bin)
oos_conf_tab
